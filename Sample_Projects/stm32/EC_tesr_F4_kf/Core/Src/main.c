...
int main(void)
{
  ...
  WheelMode(PITCH, 0);             // 模弝0：佝置模�???????? 模弝1：杒速； 模弝2：pwm调�?�；模弝3：步�????????
  WheelMode(YAW, 0);               // 模弝0：佝置模�???????? 模弝1：杒速； 模弝2：pwm调�?�；模弝3：步�????????
  ...
  while (0) {
    ...
    /* USER CODE BEGIN 3 */
        ...
        /***WitMotion***/
        RotateWithHWT();
        printf(":%f,%d,%d,%f,%d\n", fAngle[2], FT_target_angle, ReadPos(YAW), debug_kf, current_edge);
