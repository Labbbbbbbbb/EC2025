...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART_UX)             /* 如果是串口1 */
    {
				...
				else if(rxstate >=4&&rxstate <=11)  //中间8位数据
				{
					if((rxstate-4)%2==0)	uart_data [rxstate-4] = g_rx_buffer[0]>>2; //偶数位是高位数据
					else uart_data [rxstate-4] = g_rx_buffer[0];
					rxstate++;
					if(rxstate ==12) rxstate=0;
				}
